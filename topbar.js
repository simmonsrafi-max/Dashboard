...
// pushes an update to the Supabase water row so the
...
(function () {
  ...
  const TOPBAR_SUPABASE_URL = 'https://ehtaxcbwqhljfsnnludx.supabase.co';
  const TOPBAR_SUPABASE_KEY = 'sb_publishable_RTHPQaIvo4kCT8pNmOm_yA_I10i3p-k';
...
`;
  ...
  async function pushWaterToSupabase(localWater) {
    // Only do this when we're NOT on the water page — that page has its
    // own sync loop that already detects the localStorage change.
    ...
    try {
      ...
      await supa.from('app_state').upsert(
        { key: 'po_water_v1', value: localWater, updated_at: new Date().toISOString() },
        { onConflict: 'key' }
      );
    } catch (e) { /* offline — local change will sync next time the water tab is opened */ }
  }
